...
class ZoneBuilder(zones : Map[Int, Set[Coord]]= Map()) {
  ...
  def isInZone(coord: Coord): Boolean = allZonedCoords.contains(coord)
  ...
  private def findNeighbours(id: Int): Set[ZoneId] = {
    ...
    val allNeighbouringCoords = zones(id).flatMap( neighbouringCoords )
    // A neighbouring zone is a zone that contains one of the neighbouring co-ordinates
    allNeighbouringCoords.map(c => zones.find( entry => zones(entry._1).contains(c) )).flatMap( _.toList ).map( entry => entry._1).map(ZoneId)
  }
  ...
  private def edgeCoords(id: Int): Set[Coord] = {
    ...
    val edges = zones(id).filter(c => {
      ...
      if (neighbours.isEmpty) false else neighbours.exists(_ != id)
    })
...
class Zone(val id: ZoneId, val tiles: Set[Coord], val neighbours: Set[ZoneId], val edgeCoords: Set[Coord], val filteredEdgeCoords: Set[Coord]) extends Logging {
  def center: Coord = {
